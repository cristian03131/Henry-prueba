function mayorMenor(numeros) {
  ...
  function mismaCantidadCaracteres(strings, caracteres) {
    ...
    // Tu código aca:
    for(leti=0; i < strings.lenght; i++){
      if(strings[i].value.length===caracteres){
        return strings;
      }
    }
  ...
  for(let i = 0; i < amigos.nombre.propiedad.length; i++){
  if (amigos[i]!=propiedad){
  agregaPropiedad[propiedad]=null;
  }
  ...
  function crearClaseViajero() {
    class Viajero {
      ...
      addCompañero(nombre, nacionalidad, edad) {
        ...
        // Tu código aca:
        var nuevocompañero = {nombre,nacionalidad,edad}
      ...
      getCompañeros() {
        ...
        const indexed = compañeros.map (transformarObjetoCompañeroANombre);
